...
app.get("/:dynamicSegment", (req, res) => {
  ...
  fs.readFile(jsonFilePath, "utf8", (err, data) => {
    if (err) {
      console.info(
        `skeleton.json not found for ${dynamicSegment}, attempting dynamic folder lookup.`
      );
      // If skeleton.json is not found, look for dynamic folders
      const dynamicJsonFilePath = findSkeletonInDynamicFolders(
      ...
      if (dynamicJsonFilePath) {
        fs.readFile(dynamicJsonFilePath, "utf8", (err, data) => {
          if (err) {
            console.error(
              `Error reading skeleton.json from dynamic folder:`,
              err
            );
            res.status(500).json({ success: false, error: err.message });
          } else {
        ...
        res
          .status(404)
          .json({ success: false, message: "skeleton.json not found" });
      }
    } else {
      res.status(200).json(JSON.parse(data));
    }
