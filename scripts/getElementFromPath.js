...
export default function getElementFromPath(path) {
	...
	if (path) {
		if (path.includes("has")) {
            selectorLevels = path.split("has")[0].split(">");
		} else {
		...
		if (!selector.includes(":nth-of-type")) {
			...
			if (match) {
				...
				if (Number.isNaN(position)) {
					console.error(
						`Invalid position value: ${positionStr} in selector: ${selector}`,
					);
					return null;
				}
	...
	function traverseDOM(element, levels) {
		...
		const matchingElements = Array.from(element.children).filter(
	...
	const currentElement = global.id.doc.body;
